...
def get_prompt(args, graph_list, node = None, node_type = None, mediator_variables = None):

    if node_type == "source":
        node_desc = "A source node in a causal graph is a variable that does not have any incoming edges, meaning it is not caused by any other variable in the graph."
    if node_type == "sink":
        node_desc = "A sink node in a causal graph is a variable that does not have any children in the graph, meaning it is not caused by any other variables in the system."
    prompt_1 = "Hello. You will be given a causal graph. The causal relationships in this causal graph are - " + graph_represenations(args, graph_list)
    if args.numerical:
        prompt_1 = "Hello. You will be given a causal graph. The causal relationships in this causal graph are - " + graph_represenations(args, graph_list)
    if node_type == "source" or node_type == "sink":
        prompt_2 = ". Now answer using this graph, is " + node + " a " + node_type + " in the causal graph. "+ node_desc+" . Think step by step. Give reasoning and then give answer within <Answer> Yes/No </Answer>. No reasoning within the answer tags, just Yes or No. Make sure your answer is in the answer tags with list or null, nothing else."
    if node_type == "mediator":
        prompt_2 = " Now answer using this graph, is " + node + " a " + node_type + " for "+ mediator_variables[0] + " and " + mediator_variables[1] + ". Only consider mediators that exist in the direct causal path (not mediated via other mediators). Think step by step. Give reasoning and then give answer within <Answer> Yes/No </Answer>.Make sure your answer is in the answer tags with list or null, nothing else."
    if args.interv:
        prompt_2 = " Now answer using this graph, Suppose we perform the perfect causal intervention, do( " + node + ") i.e this node is removed. After intervention, the causal graph changes. Now using this intervened causal graph answer whether does "+ mediator_variables[1] + " cause a change in " + mediator_variables[2] + ". Think step by step. Give reasoning and then give answer within <Answer> Yes/No </Answer>.Make sure your answer is in the answer tags with list or null, nothing else."
...
def prompt_graph_detail(args, graph_list, described_nodes, node_type):
    ...
    for node in described_nodes:
        all_p = get_prompt(args, graph_list, node=node, node_type=node_type, mediator_variables=None)
...
def prompt_graph_intervention(args, graph_list, described_nodes, node_type, interventions):
    full_prompt = []
    # breakpoint()
    for ivs in interventions:
        for i in ivs:
            all_p = get_prompt(args, graph_list, node=i[0], node_type=node_type, mediator_variables=i)
            full_prompt.append(all_p)
    return full_prompt
