...
def prompt_gpt(prompts, gpt, temp, max_tokens, top_p, frequency_penalty, presence_penalty):
    ...
    conversation=[{"role": "system", "content": "Strictly follow the format to return the answer. If your answer is a1, a2 and a3, then return the answer within <Answer>[a1 , a2, a3]</Answer>, if Null then return <Answer>Null</Answer>"}]
    ans_dict = {}
...
def get_prompt(args, graph_list, node = None, node_type = None, mediator_variables = None):
    ...
    if node_type == "source" or node_type == "sink":
        ...
        prompt_2 = ". Now answer using this graph only, name all of the " + node_type + " in the causal graph. "+ node_desc+" . Think step by step. Give reasoning and then give answer within <Answer> [a1,a2,a3..] </Answer>, if Null then return <Answer>Null</Answer>. Make sure your answer is in the answer tags with list or null, nothing else."
    if node_type == "mediator":
        prompt_1 = "Hello. You will be given a causal graph." + descp +  "The causal relationships in this causal graph are - " + graph_represenations(args,graph_list)
        prompt_2 = ". Now answer using this graph only, give all the " + node_type + " between "+ mediator_variables[0] + " and " + mediator_variables[1] + ". Only consider mediators that exist in the direct causal path (not mediated via other mediators). Think step by step. Give reasoning and then give answer within <Answer> [a1,a2,a3..] </Answer>, if Null then return <Answer>Null</Answer>.Make sure your answer is in the answer tags as prescribed earlier with list or null, nothing else."
    return [prompt_1 + prompt_2]

def prompt_graph_detail(args, graph_list, described_nodes, node_type):
    full_prompt = []
    all_p = get_prompt(args, graph_list, node_type=node_type, mediator_variables=None)
...
def prompt_graph_detail_mediator(args, graph_list, described_nodes, node_type, mediator_variables):
    ...
    for key, item in mediator_variables.items():
        all_p = get_prompt(args, graph_list, node=key, node_type=node_type, mediator_variables=key)
        full_prompt.append(all_p)
    return full_prompt
...
def open_world_sink_nodes_prompt(args, graph_list, missing_nodes, codebook):
    ...
    for i, (key, value) in enumerate(missing_nodes.items()):
        starting = "Hello. You will be given a causal graph. The context of the graph is " + description(args) + ". Please understand the causal relationships between the variables - " + graph_represenations(graph_list[i])
